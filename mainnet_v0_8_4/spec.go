...
func (*CommitteePubkeys) Limit() uint64 {
	return MAX_VALIDATORS_PER_COMMITTEE
}

type CommitteeCompacts []uint64

func (*CommitteeCompacts) Limit() uint64 {
	return MAX_VALIDATORS_PER_COMMITTEE
}

type CompactCommittee struct {
	Pubkeys           CommitteePubkeys
	CompactValidators CommitteeCompacts
}
...
type BeaconState struct {
	...
	ActiveIndexRoots       [EPOCHS_PER_HISTORICAL_VECTOR]Hash // Active index digests for light clients
	CompactCommitteesRoots [EPOCHS_PER_HISTORICAL_VECTOR]Hash // Committee digests for light clients
	// Slashings
	...
	// Crosslinks
	PreviousCrosslinks [SHARD_COUNT]Crosslink // Previous epoch snapshot
